        ...
        set((state) => {
          const newBokses = state.bokses.map((boks) => {
            if (boks.id === state.currentBoksId) {
              const noteIndex = boks.notes.findIndex(
        ...
        set((state) => {
          ...
          const newBoks = {
            ...
            notes: data.notes.map((note) => ({
              ...note,
              // FIX: Keep the original ID (like "vp0012") if it exists!
              id: note.id ? note.id : `${newBoksId}-${Math.random().toString(36).substr(2, 5)}`,
            })),
          };

          return {
            bokses: [...state.bokses, newBoks],
            currentBoksId: newBoksId, 
          };
        }),
      clearAll: () =>
        set((state) => {
          ...
          const fallbackTutorial = {
            ...
            name: "Tutorial Boks",
            notes: [
              /* ... your new tutorial notes array from earlier ... */
            ],
          };
